...
<script src="<%=request.getContextPath()%>/pages/studentPages/js/jquery.kwicks-1.5.1.pack.js" type="text/javascript"></script>
  ...
  <% if (session.getAttribute("loginedStudent") == null) { %>
	  alert("Only open to platform user.Please retry after logged in.");
	  window.location = "<%=request.getContextPath()%>/pages/studentPages/studentPage.jsp";
  <% } %>
  $(function(){
...
var literatureGrid = new Ext.grid.GridPanel({
	...
	listeners:
	{
		cellclick:function(obj, rowIndex, columnIndex, f)
		{
			...
			//alert(url);
			//alert(columnIndex);
			//alert(url+"/elibrary/uploadedPDF/"+id+ "FilePDF.pdf");
			//console.log(id);
			if(columnIndex == 8)
			{//FileIMG
...
<object id="actualpdf" data="<%=request.getContextPath()%>/PDF/test.pdf" type="application/pdf" width="100%" height="100%">
  ...
  No biggie... you can <a id="altPDF" href="myfile.pdf">click here to
